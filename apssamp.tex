...
\textit{Niyaz / Readout scheme with superconducting resonator, dispersive readout, estimate coupling constants}
...
\section{Entanglement in quantum dot systems} % Hakon, Oyvind
    ...
    \subsection{Measuring the degree of entanglement}
        ...
        \begin{gather*}
            \hat{\rho}^L
            = \tr_R \! \qty(\hat{\rho})
            = \sum_{i, k = 1}^{N_L} \sum_{j = 1}^{N_R}
            C_{ij} C^{*}_{kj} \dyad*{\phi^L_i}{\phi^L_k},
            \\
            \hat{\rho}^R
            = \tr_{L} \! \qty(\hat{\rho})
            = \sum_{i = 1}^{N_L} \sum_{j, l = 1}^{N_R}
